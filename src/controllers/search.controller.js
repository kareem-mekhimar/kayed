...
export default {
    async search(req, res, next) {
        ...
        try{    
            ...
            let minVal = Math.min(barterCount, auctionCount); 
            let maxVal = Math.max(barterCount, auctionCount);

            let pageCount = Math.ceil(maxVal / limit) * 2 - Math.ceil( Math.abs(barterCount - auctionCount) / limit );
            let halfLimit = limit? Math.ceil( limit/2 ) : 5;

            if(page > Math.ceil((minVal / limit) * 2))
                halfLimit = limit;
